class Screen
  ...
  def letters(width)
     (@array[0].length / width).times do |i|
      puts to_s(@array.transpose[(width * i)...((width * i) + width)].transpose)
      puts
     end
  end

  def to_s(array = @array)
    output = []
    array.length.times do |y|
       output << array[y].map{|v|v ? "#" : "."}.join
    end
    output.join("\n")
  end
end
