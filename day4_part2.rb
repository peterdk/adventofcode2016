...
attr_reader :checksum, :sector

  def initialize(input)
    # puts input
    @checksum = input.match(/\[(\w+)\]/)[1]
    @sector = input.match(/(\d+)\[/)[1].to_i
    @encrypted =input.match(/^[a-z-]+/)[0]
    @letters = @encrypted.gsub("-","")
    # puts @checksum
    # puts @sector
    # puts @letters
  end
  ...
  def calc_checksum
    grouped = @letters.chars.group_by{|c|c}.map{|k,v|[k, v.count]}.sort_by{|c|c[1]}.reverse
    sorted = grouped.group_by{|g|g[1]}.map{|k,v|[k, v.map{|i|i[0]}.sort]}
    sorted.map{|s|s[1]}.flatten.take(5).join
  end

  def decrypt
    decrypted = @encrypted.chars.map do |c|
      if (c == "-")
        " "
      else
        ord = c.ord + (@sector % 26)
...
input1 = "aaaaa-bbb-z-y-x-123[abxyz]"
input2 = "a-b-c-d-e-f-g-h-987[abcde]"
input3 = "not-a-real-room-404[oarel]"
input4 = "totally-real-room-200[decoy]"
input5 = "qzmt-zixmtkozy-ivhz-343[abcde]"
test_input = [input1, input2, input3, input4, input5].join("\n")

Processor.new.process(test_input)

real_input ="vxupkizork-sgmtkzoi-pkrrehkgt-zxgototm-644[kotgr]
...
shoewudys-tou-ixyffydw-478[uszty]"
 ...
 Processor.new.process(real_input)
